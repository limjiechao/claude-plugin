...
class SettingsReconcileTests(unittest.TestCase):
        ...
        with tempfile.TemporaryDirectory() as tmp:
            ...
            old_hook = {
                "PreToolUse": [
                    {
                        "matcher": "Bash",
                        "hooks": [{"type": "command", "command": "/tmp/claude/hooks/git-guard.sh"}],
                    }
                ]
            }
            settings.write_text(json.dumps({"hooks": old_hook, "userKey": "keep"}))
                            ...
                            "PreToolUse": [
                                {
                                    "matcher": "Bash",
                                    "hooks": [
                                        {"type": "command", "command": "@@HOOKS_DIR@@/git-guard.sh"}
                                    ],
                                }
                            ]
                        }
    ...
    def test_replaces_tokens_and_forces_statusline(self):
        with tempfile.TemporaryDirectory() as tmp:
            ...
            snippet.write_text(
                json.dumps(
                    {"hooks": {"PreToolUse": [{"hooks": [{"command": "@@HOOKS_DIR@@/git-guard.sh"}]}]}}
                )
            )
            ...
            self.assertEqual(
                ...
                "/tmp/claude/hooks/git-guard.sh",
            )
            self.assertEqual(
                merged["statusLine"],
                {"type": "command", "command": "bash /tmp/claude/statusline.sh"},
            )
            self.assertEqual(json.loads(last.read_text()), json.loads(snippet.read_text()))
