...
deny_patterns=(
  ...
  # Protect guard infrastructure & secrets (literal ~, $HOME, and absolute home).
  'rm[[:space:]]+[^|&;]*(~|\$HOME|/Users/jiechao)/\.(claude|ssh)'                            # rm of ~/.claude or ~/.ssh
  '(>|>>)[[:space:]]*[^|&;]*(~|\$HOME|/Users/jiechao)/\.(claude|ssh|zshrc|bashrc|gitconfig)' # clobber/append into sensitive paths
  '(mv|cp|tee)[[:space:]]+[^|&;]*(~|\$HOME|/Users/jiechao)/\.(claude|ssh|zshrc)'             # mv/cp/tee over sensitive paths
