...
> Principle: **reference, never vendor.** You add a pointer; `install.sh` fetches it.
...
2. Install it: `./bootstrap/install.sh` (or, ad hoc in Claude Code,
...
Then `./bootstrap/install.sh` (clones the repo, copies the skill folder into
...
`bootstrap/mcp.json` + a merge step in `install.sh` — not present yet; add it the
...
> are delivered by `install.sh` into `~/.claude` — because plugin-bundled subagents
...
1. Create `bootstrap/agents/<name>.md`:
   ...
   tools: Bash, Read
   model: inherit
   permissionMode: dontAsk      # honored because install.sh puts this in ~/.claude/agents
...
2. **Invariant:** an agent's hook path uses the `@@HOOKS_DIR@@` token (rewritten by
   `install.sh` to `$HOME/.claude/hooks`), never `/Users/...` and never
   `${CLAUDE_PLUGIN_ROOT}` (agents aren't shipped via the plugin).
3. Run `./bootstrap/install.sh` (copies it to `~/.claude/agents`). Commit + push.

### 2c. A personal hook

1. Add the script: `bootstrap/hooks/<name>.sh` (`chmod +x`).
2. Reference it from a subagent (2b) as `@@HOOKS_DIR@@/<name>.sh`.
3. Run `./bootstrap/install.sh` (copies it to `~/.claude/hooks`). Commit + push.
...
2. `./bootstrap/install.sh` re-merges into `~/.claude/settings.json` (backed up
...
| Personal subagent | `bootstrap/agents/<name>.md` (use `@@HOOKS_DIR@@`) | `./bootstrap/install.sh` | B |
| Personal hook | `bootstrap/hooks/<name>.sh` | `./bootstrap/install.sh` | B |
| Personal MCP | `plugins/jiechao-toolkit/.mcp.json` | `/plugin update jiechao-toolkit` | A |
| Settings/statusline | `bootstrap/settings.snippet.json` / `statusline.sh` | `./bootstrap/install.sh` | B |
| Third-party plugin | `bootstrap/manifest.json` | `./bootstrap/install.sh` | B |
| Third-party skill | `bootstrap/skills.lock.json` | `./bootstrap/install.sh` | B |
| Third-party MCP | add its plugin in `manifest.json` | `./bootstrap/install.sh` | B |

## Don'ts

- ❌ Don't hand-edit a skill/agent in `~/.claude` and consider it done — it will be
  overwritten on the next rebuild. Mirror it into the repo (or run `export.sh`).
...
- ❌ Don't replace `~/.claude/settings.json` — only merge via `install.sh`.
